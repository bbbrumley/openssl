...
EVP_CipherUpdate(), EVP_EncryptUpdate() or EVP_DecryptUpdate() should be made
with I<out> set to a non-NULL value, and all AAD must be supplied before it.
A further AAD or data update after that call fails with
B<PROV_R_UPDATE_CALL_OUT_OF_ORDER>. When decrypting, the tag is verified by
EVP_DecryptFinal() or EVP_CipherFinal(), which must therefore be called.
